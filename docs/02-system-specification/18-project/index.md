...
1. プロジェクト開始
   ├ スキル「特定技能申請」を選択
   ├ 対象学生を選択（在校生 → 既存データが自動入力済み）
   └ 受入れ企業を選択/新規登録

2. タスクボード（WBS）が自動生成される
   ┌──────────────────────────────────────────────────┐
   │ 書類準備タスク          ステータス  担当  メモ    │
   │ ☑ DOC-001 申請書(申請人用) 自動入力済  —   91%自動 │
   │ ☑ DOC-003 雇用契約書      完了       佐藤        │
   │ ☐ COL-001 健康診断        収集依頼中  田中 予約済 │
   │ ☐ COL-003 技能試験合格証   不要       —   免除   │
   │ ☐ COL-012 登記事項証明書   未着手     —          │
   │ ...                                              │
   └──────────────────────────────────────────────────┘
...
プロジェクト開始時に、学生の属性（国籍・技能実習修了有無・保険種別）と分野に基づいて、必要な書類を自動判定する。

- 技能実習2号修了者 → 技能試験・日本語試験は「不要」
- 介護分野 → 介護日本語評価試験合格証が「必須」に追加
- ベトナム国籍 → 二国間取決書類が「必須」に追加

詳細は [特定技能データ定義 > 条件分岐ルール](../../01-domain-knowledge/15-specified-skilled-worker/02-data.md) を参照。
...
| **条件分岐ルール** | タスクの必要/不要判定 | 技能実習修了者は試験免除 |
...
| ゴール | 入管への申請書類の提出 |
| 対象者 | 常南国際学院の在校生/卒業生 |
| タスク数 | 最大36件（DOC-001〜013 + COL-001〜023）※条件分岐で変動 |
