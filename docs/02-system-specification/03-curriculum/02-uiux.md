...
| 画面名 | パス | 役割 |
|--------|------|------|
| コース一覧 | /curriculum/courses | コースの一覧表示・管理 |
| コース詳細 | /curriculum/courses/:id | コース情報の表示・編集 |
| コース登録 | /curriculum/courses/new | 新規コースの登録 |
...
- 参照エンティティ: Course
  - 表示プロパティ: name, targetLevel, durationMonths, annualHours, isActive
- フィルタ:
  - 対象レベル（JlptLevel）
  - 有効/無効
- ソート: コース名（デフォルト昇順）
...
- インライン編集可能
- annualHours は 760 未満を設定しようとした場合、警告を表示
...
|-----------|-----|------|---------------|------|
| コース名 | text | o | 1文字以上、重複不可 | |
| 対象JLPTレベル | select | o | JlptLevel Enum | |
| コース期間（月数） | number | o | 1以上 | |
| 年間授業時間 | number | o | 760以上 | 法定基準 |
| 説明 | textarea | - | | |
