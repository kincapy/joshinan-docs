...
| 在籍解除ボタン | - （画面遷移なし） | 在籍学生一覧の各行に表示 |

#### 在籍解除の操作フロー

1. 在籍学生一覧の対象学生の行にある「在籍解除」ボタンをクリック
2. 確認ダイアログを表示: 「{学生名} をこのクラスから在籍解除しますか？」
3. ユーザーが「解除する」を選択した場合:
   - 対象の ClassEnrollment の `endDate` を本日の日付で設定
   - 対象の ClassEnrollment の `status` を `ENDED` に変更
4. 在籍学生一覧から対象学生が非表示になる（endDate が設定されたため）
