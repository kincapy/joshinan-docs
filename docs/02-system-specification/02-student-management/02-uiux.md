...
- 参照エンティティ: Student, StudentEmployment, InterviewRecord, StudentCertification
- タブ構成:

| タブ | 表示内容 |
|------|---------|
| 基本情報 | 氏名、生年月日、性別、国籍、連絡先、住所、備考（notes） |
| 学籍情報 | 学籍番号、ステータス、コホート、現在のクラス（ClassEnrollment経由）、入学日、卒業予定日、エージェント名（agentName）、退学日（withdrawalDate）、退学理由（withdrawalReason） |
| 在留情報 | パスポート、在留カード、在留資格、在留期限、資格外活動許可 |
| 入国情報 | 入国日、入国空港、便名、出迎え担当 |
| 勤務先 | 勤務先一覧（最大3件）、週合計勤務時間 |
| 進路 | 進路区分、進路先名、合否 |
| 資格証 | 資格証一覧（StudentCertification）→ 新規登録フォーム |
...
|-----------|-----|------|---------------|------|
| 試験種別 | select | o | ExamType Enum | |
| レベル・級 | text | - | | 試験種別によって形式が異なる |
| 受験日 | date | o | 過去〜本日 | |
| 合否 | select | o | ExamResult Enum | |
| スコア | number | - | 0以上の整数 | 点数がある試験の場合 |
| 証明書番号 | text | - | | |
| 備考 | textarea | - | | |
...
| プロパティ | 表示タブ | 備考 |
|-----------|---------|------|
| エージェント名（agentName） | 学籍情報 | Agent テーブルから名前を表示。編集不可（保護対象） |
| 退学日（withdrawalDate） | 学籍情報 | ステータスが WITHDRAWN / EXPELLED の場合のみ表示。編集不可（保護対象） |
| 退学理由（withdrawalReason） | 学籍情報 | ステータスが WITHDRAWN / EXPELLED の場合のみ表示 |
| 備考（notes） | 基本情報 | 自由記述テキストエリア |
