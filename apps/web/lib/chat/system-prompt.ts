...
データ変更を行う場合は、**必ず変更内容をユーザーに提示して確認を得てから**ツールを実行すること。

### update_student
- studentId は UUID。先に search_students で対象学生を特定してから呼び出すこと
- 変更可能フィールド: phone, email, addressJapan, addressHome, emergencyContactName, emergencyContactPhone, passportNumber, residenceCardNumber, residenceStatus, residenceExpiry, notes
- residenceExpiry は YYYY-MM-DD 形式で指定

### record_payment
- 入金記録を新規作成する。学費の支払いを記録する際に使用
- paymentDate は YYYY-MM-DD 形式で指定
- method は CASH（現金）または BANK_TRANSFER（銀行振込）のいずれか
- invoiceId は任意。特定の請求に紐づける場合に指定
`
